		...
		{   
			...
			if(rede[y][x]->buffer_local_entrada->size > 0) {
				...
				}  else if (rede[y][x]->roteamento_local.portaDestino == LESTE)
				{
					...
					if (rede[y][x+1]->cf_oeste->in_ack.read() != 1){
			...
			if (ver_sul[y][x])
			{
				...
				if (rede[y][x]->roteamento_sul.portaDestino == LOCAL)
				{
					...
					cout << "ciclos " << clock<<endl;
					if (rede[y][x]->buffer_sul->flits.front().end != -1)
					{
						cout << " end:"<< rede[y][x]->buffer_sul->flits.front().end <<"   "<< clock <<" "<<pacotes_tg[rede[y][x]->buffer_sul->flits.front().end].first_flit<<endl;
